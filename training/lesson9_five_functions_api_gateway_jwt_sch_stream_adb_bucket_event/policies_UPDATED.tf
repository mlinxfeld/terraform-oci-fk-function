...
resource "oci_identity_policy" "FoggyKitchenFnOSSPolicy" {
  provider = oci.homeregion  
  name = "FoggyKitchenFnOSSPolicy"
  description = "FoggyKitchenFnOSSPolicy"
  compartment_id = var.tenancy_ocid

  statements = [
    "Allow dynamic-group ${oci_identity_dynamic_group.FoggyKitchenFunctionDG.name} to read buckets in compartment id ${var.compartment_ocid}"
  ]
}